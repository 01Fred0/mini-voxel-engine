    ...
    for (let x = 0; x < chunk.size; x++) {
      for (let z = 0; z < chunk.size; z++) {
        ...
        for (let y = chunk.height - 1; y >= 0; y--) {
          ...
          if (blockType === BlockTypes.AIR || !block || !block.solid) {
  ...
  propagateLightHorizontal(chunk, isSkylight) {
    ...
    while (queue.length > 0) {
      ...
      for (const [nx, ny, nz] of neighbors) {
        ...
        const block = Block.getBlock(neighborBlock);
        
        // Only propagate through air/transparent blocks
        if (neighborBlock !== BlockTypes.AIR && block && block.solid) continue;
  ...
  propagateLightFromSource(chunk, x, y, z, lightLevel, isSkylight) {
    ...
    while (queue.length > 0) {
      ...
      for (const [nx, ny, nz] of neighbors) {
        ...
        if (blockType !== BlockTypes.AIR && block && block.solid) continue;
